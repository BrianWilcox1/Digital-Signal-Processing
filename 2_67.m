...
h4_temp = zeros(length(n) + 2);
for i =1:length(n)
    %for h4_temp we are storing values from 3 to 102 and accessing old
    %values including h4_temp(1) and h4_temp(2)
    %del is the function for delta. We want a delta at n = 0 and n =2. The
    %indexing for del must be subtracted by one because of Matlab indexing
    hw_temp(i+2) = 0.9*hw_temp(i+1) - 0.81 * hw_temp(i) +del(i - 1)+del(i - 2);
end
